...
const route = getRouteApi("/_dashboard/$mailboxId/inbox/starred");

export default function StarredPage() {
  const { mailboxId } = route.useParams();
  const initialPage = route.useLoaderData();
  const emailData = useEmailData({ view: "starred", mailboxId, initialPage });
  const { openEmail, executeEmailAction } = useEmailInboxActions({ view: "starred", mailboxId });
