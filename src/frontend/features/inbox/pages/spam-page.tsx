...
const route = getRouteApi("/_dashboard/$mailboxId/inbox/spam");

export default function SpamPage() {
  const { mailboxId } = route.useParams();
  const initialPage = route.useLoaderData();
  const emailData = useEmailData({ view: "spam", mailboxId, initialPage });
  const { openEmail, executeEmailAction } = useEmailInboxActions({ view: "spam", mailboxId });
