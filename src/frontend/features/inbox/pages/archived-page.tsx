...
const route = getRouteApi("/_dashboard/$mailboxId/inbox/archived");

export default function ArchivedPage() {
  const { mailboxId } = route.useParams();
  const initialPage = route.useLoaderData();
  const emailData = useEmailData({ view: "archived", mailboxId, initialPage });
  const { openEmail, executeEmailAction } = useEmailInboxActions({ view: "archived", mailboxId });
