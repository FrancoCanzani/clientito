...
const route = getRouteApi("/_dashboard/$mailboxId/inbox/trash");

export default function TrashPage() {
  const { mailboxId } = route.useParams();
  const initialPage = route.useLoaderData();
  const emailData = useEmailData({ view: "trash", mailboxId, initialPage });
  const { openEmail, executeEmailAction } = useEmailInboxActions({ view: "trash", mailboxId });
