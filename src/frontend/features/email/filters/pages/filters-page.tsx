...
import {
  FilterEditor,
  type EditingFilter,
} from "@/features/email/filters/components/filter-editor";
import {
  createFilter,
  deleteFilter,
  generateFilter,
  updateFilter,
} from "@/features/email/filters/queries";
import type { EmailFilter } from "@/features/email/filters/types";
import {
  ArrowRightIcon,
  PencilSimpleIcon,
  PlusIcon,
...
export default function FiltersPage() {
  ...
  const availableTemplates = useMemo(() => {
    const existingNames = new Set(loadedFilters.map((filter) => filter.name));
  ...
  return (
    <div className="flex min-h-0 w-full max-w-2xl min-w-0 flex-1 flex-col gap-8">
      ...
      {generateMutation.isPending && loadedFilters.length === 0 ? (
      ...
      ) : loadedFilters.length === 0 && !editing ? (
