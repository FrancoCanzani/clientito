...
const TABS = [
  ...
  { id: "reminders", label: "Reminders", to: "/$mailboxId/reminders" as const },
  ...
  return useRouterState({
    select: (state) => {
      ...
      if (matches.some((m) => m.routeId === "/_dashboard/$mailboxId/todo"))
        return "todo";
      if (matches.some((m) => m.routeId === "/_dashboard/$mailboxId/reminders"))
        return "reminders";
...
export function InboxHeaderShell({ children }: { children: ReactNode }) {
  return (
    <div className="flex h-full min-h-0 flex-1 flex-col overflow-hidden">
      <TopBar />
      <div className="mx-auto flex min-h-0 w-full flex-1 flex-col overflow-hidden p-3">
        {children}
      </div>
    </div>
  );
