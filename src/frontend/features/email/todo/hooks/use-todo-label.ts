...
import {
  ...
  createLabel,
  removeLabel,
} from "@/features/email/labels/mutations";
import {
  fetchLabels,
  syncLabelsFromServer,
} from "@/features/email/labels/queries";
...
export function findTodoLabel(labels: Label[]): Label | null {
  ...
  return (
    labels.find((label) => label.name.toLowerCase() === normalizedName) ?? null
  );
}
...
export function useTodoLabel(
  ...
  const labelQuery = useQuery({
    ...
    queryFn: () =>
      options.autoCreate
        ? ensureTodoLabel(mailboxId)
        : fetchTodoLabel(mailboxId),
    staleTime: 5 * 60_000,
  });
