...
>();
const defaultTemplateDateFormatters = {
  date: Intl.DateTimeFormat(undefined, { dateStyle: "long" }),
  day: Intl.DateTimeFormat(undefined, { weekday: "long" }),
  time: Intl.DateTimeFormat(undefined, { timeStyle: "short" }),
};
...
function deriveName(name: string, email: string): string {
  ...
  for (const part of local.split(/[._-]+/)) {
    if (part) nameParts.push(part.charAt(0).toUpperCase() + part.slice(1));
  }
...
function getTemplateDateFormatters(locale?: string) {
  if (!locale) return defaultTemplateDateFormatters;
  const key = locale ?? "";
  const cached = formatterCache.get(key);
  if (cached) return cached;

  const formatters = {
    date: Intl.DateTimeFormat(locale, { dateStyle: "long" }),
    day: Intl.DateTimeFormat(locale, { weekday: "long" }),
    time: Intl.DateTimeFormat(locale, { timeStyle: "short" }),
  };
...
export function buildTemplateContext(input: {
  ...
  const formatters = getTemplateDateFormatters(locale);
