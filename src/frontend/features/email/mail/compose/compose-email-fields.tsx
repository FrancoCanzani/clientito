...
function ForwardedMessagePreview({ html }: { html: string }) {
  ...
  return (
    <div className="mt-4 pt-2">
      <button
        type="button"
        className="flex h-5 items-center bg-muted px-2 text-[10px] font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-accent-foreground"
        onClick={() => setExpanded((v) => !v)}
        aria-expanded={expanded}
        aria-label={
          expanded ? "Hide forwarded message" : "Show forwarded message"
        }
      >
        &hellip;
      </button>
...
export function ComposeEmailFields({
  ...
  const aiEnabledForMailbox =
    availableMailboxes.find((m) => m.mailboxId === mailboxId)?.aiEnabled ??
    false;
  ...
  useEffect(() => {
    if (aiEnabledForMailbox) return;
    if (reviewState.status !== "idle") {
      setReviewState({ status: "idle" });
    }
  }, [aiEnabledForMailbox, reviewState.status]);
      ...
      <div className="relative mt-auto p-2">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1">
            ...
            <ReplyReminderPicker
              value={replyReminderMs}
              onChange={setReplyReminderMs}
            >
              <Button
                variant="ghost"
                size="icon"
                ...
                aria-label={
                  replyReminderMs
                    ? `Reply reminder set: ${formatReminderLabel(replyReminderMs)}`
                    : "Add to to-do if no reply"
                }
                title={
                  replyReminderMs
                    ? `Remind in ${formatReminderLabel(replyReminderMs)} if no reply`
                    : "Add to to-do if no reply"
                }
              >
                <ListChecksIcon
                  className={cn("size-3", replyReminderMs && "text-foreground")}
                  weight={replyReminderMs ? "fill" : "regular"}
                />
              </Button>
            </ReplyReminderPicker>
          </div>
