...
import { invalidateInboxQueries, invalidateEmailDetail, invalidateEmailThread } from "@/features/email/mail/shared/data/invalidation";
import {
  applyMailPatchToCaches,
  applyPatchToLabelIds,
  type EmailPatchPayload,
} from "@/features/email/mail/shared/utils/optimistic-mail-state";
...
function toNumericIds(ids: string[]): number[] {
  return ids.map((id) => Number(id)).filter((value) => Number.isFinite(value));
}
...
function computeEmailUnreadDelta(
  ...
  return (
    Number(isUnreadInboxLabelSet(after)) - Number(isUnreadInboxLabelSet(before))
  );
}
...
function computeUnreadDeltaForThread(
  ...
  const source =
    emails.length > 0 ? emails : [{ labelIds: fallbackLabelIds ?? [] }];
...
function applyInboxUnreadDelta(mailboxId: number, delta: InboxUnreadDelta) {
  ...
  queryClient.setQueryData<InboxUnreadCount | undefined>(
    ...
    (current) => {
      ...
      return {
        ...
        threadsUnread: Math.max(0, current.threadsUnread + delta.threadsUnread),
      };
    },
  );
...
function isIdempotentReadOnlyPatch(data: EmailPatchPayload): boolean {
  const keys = Object.keys(data);
  return keys.length === 1 && keys[0] === "isRead";
}
...
async function patchEmails(
  ...
  const beforeLocal = await localDb.getEmailsByProviderMessageIds(
    userId,
    mailboxId,
    providerIds,
  );
  ...
  try {
    ...
    const retries = isIdempotentReadOnlyPatch(data) ? 2 : 0;
    if (emails.length === 1) {
      const email = emails[0]!;
      const response = await fetchWithRetry(
        () =>
          fetch(`/api/inbox/emails/${email.id}`, {
            method: "PATCH",
      ...
      if (!response.ok) {
        const err = await responseError(response, "Failed to update email");
        console.warn("[patchEmail] failed", {
          emailId: email.id,
      ...
      const groups = new Map<
        string,
        { labelIds: string[]; emails: EmailIdentifier[] }
      >();
      for (const email of emails) {
        const labelIds = email.labelIds ?? [];
        const key = JSON.stringify(labelIds);
        const bucket = groups.get(key);
        if (bucket) bucket.emails.push(email);
        else groups.set(key, { labelIds, emails: [email] });
      }

      for (const { labelIds, emails } of groups.values()) {
        ...
        if (!response.ok) {
          const err = await responseError(response, "Failed to update emails");
          console.warn("[patchEmails] batch failed", {
    ...
    if (!requestSucceeded) {
      ...
      if (isIdempotentReadOnlyPatch(data)) {
...
export async function patchThread(
  ...
  try {
    ...
    if (!response.ok)
      throw await responseError(response, "Failed to update thread");
    await localPatchTask;
    requestSucceeded = true;
  } finally {
    await localPatchTask;
...
export async function createReplyReminder(
  ...
  const json = (await response.json().catch(() => null)) as {
    data?: CreateReplyReminderResult;
    error?: string;
  } | null;
