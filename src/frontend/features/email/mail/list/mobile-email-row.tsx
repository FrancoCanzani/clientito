...
const SWIPE_SETTLE_TRANSITION = "transform 240ms cubic-bezier(0.2, 0.8, 0.2, 1)";
...
export const MobileEmailRow = memo(function MobileEmailRow({
 ...
 const ids = useMemo(() => props.group.emails.map((item) => item.id), [props.group.emails]);
 ...
 (action: "archive" | "move-to-inbox" | "not-spam" | "mark-read" | "mark-unread") => {
 ...
 const primaryStyle = getSwipeActionStyle(primaryAction.action, reachedThreshold);
 const secondaryStyle = getSwipeActionStyle(secondaryAction.action, reachedThreshold);
 ...
 const clamped = Math.max(-SWIPE_MAX_DISTANCE, Math.min(SWIPE_MAX_DISTANCE, dx));
 const nextOffset =
 Math.sign(clamped) *
 ...
 const finalizeGesture = useCallback((event: PointerEvent<HTMLDivElement>) => {
 if (pointerIdRef.current !== event.pointerId) return;
 ...
 }, [closeSwipe, primaryAction.action, secondaryAction.action, setSwipeOffset, triggerAction]);
 ...
 {!email.isRead && <span className="size-1.5 shrink-0 bg-primary" aria-hidden />}

 <div className="ml-auto flex shrink-0 items-center gap-1.5 text-xs text-muted-foreground">
 {hasMetaIcons && (
 <div className="flex shrink-0 items-center gap-1">
 {isStarred && <StarIcon className="size-3 text-yellow-400" weight="fill" aria-hidden />}
 {email.hasCalendar && <CalendarIcon className="size-3" aria-hidden />}
 {email.hasAttachment && <PaperclipIcon className="size-3" aria-hidden />}
 </div>
 )}
 {threadCount > 1 && <span className="text-[11px] tabular-nums">[{threadCount}]</span>}
...
function getSwipeActionStyle(
 action: "archive" | "move-to-inbox" | "not-spam" | "mark-read" | "mark-unread" | "snooze",
 active: boolean,
) {
 switch (action) {
 case "archive":
