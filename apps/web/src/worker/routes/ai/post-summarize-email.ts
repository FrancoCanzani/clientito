...
const summarizeEmailResponseSchema = z.object({
  data: z.object({
    summary: z.string(),
  }),
});

const postSummarizeEmailRoute = createRoute({
  method: "post",
  ...
  request: {
    body: {
      content: {
        "application/json": { schema: summarizeEmailBodySchema },
      },
      required: true,
    },
  },
  responses: {
    200: {
      content: {
        "application/json": { schema: summarizeEmailResponseSchema },
      },
...
export function registerPostSummarizeEmail(app: OpenAPIHono<AppRouteEnv>) {
  app.openapi(postSummarizeEmailRoute, async (c) => {
    const db = c.get("db");
